...
import sys
sys.path.append('/home/jefri/ros2_ws/src/my_robot_interfaces')
...
class TurtleSpawner(Node):
    def __init__(self):
        ...
        self.alive_turtles_ = []
        self.alive_turtles_publisher_ = self.create_publisher(TurtleArray, "alive_turtle", 10)
    ...
    def callback_call_spawn(self, future, turtle_name, x, y, theta):
        try:
            ...
            if response.name:
                self.get_logger().info(f"Turtle {response.name} is now alive at ({x}, {y}, {theta})")
                new_turtle = Turtle()
